...
def main():
    ...
    transform = ToTensor()
    train_set = ImageTrafficSignsDataset(x_train, y_train, transform)
    val_set = ImageTrafficSignsDataset(x_valid, y_valid, transform)
    test_set = ImageTrafficSignsDataset(x_test, y_test, transform)
    ...
    run['sys/tags'].add([f'loss: CrossEntropyLoss'])
    run['sys/tags'].add([f'optmizer: Adam'])
    weights.reset_seeds()

    mlp  = ConvolutionalNeuralNetwork(output_size=hparams['categories'])
    ...
    # Optmization
    optimizer = torch.optim.Adam(mlp.parameters(), lr = hparams['learning_rate'], weight_decay=0.0001)
